...
class MatterportDataset:
    ...
    def get_best_view(self, obj_id):
        ...
        """
        obj_dict = np.load(os.path.join(self.object_dict_dir, 'object_dict.npy'), allow_pickle=True).item()
        if obj_id not in obj_dict or 'best_view' not in obj_dict[obj_id]:
            return None, None
            
        view_info = obj_dict[obj_id]['best_view']
        rgb_image = cv2.imread(os.path.join(os.path.dirname(self.root), view_info['image_path']))
